...
def create_mask(src, tgt, exit_particle, pad_value, device):
    """
    ...
    - n_exit_particle: Number of exiting particles.
    ...
    """
    src_seq_len = src.shape[0] + 1 + exit_particle.shape[0]
    tgt_seq_len = tgt.shape[0] + 1

    tgt_mask = generate_square_subsequent_mask(tgt_seq_len, device)
    src_mask = torch.zeros((src_seq_len, src_seq_len), device=device).type(torch.bool)

    src_padding_mask = torch.zeros(size=(src.size(1), src.size(0)+1+exit_particle.shape[0]), dtype=torch.bool).to(device)
    src_padding_mask[:, 1+exit_particle.shape[0]:] = (src[:, :, 0] == pad_value).transpose(0, 1)
    src_padding_mask[:, 1:1+exit_particle.shape[0]] = (exit_particle[:, :, 0] == pad_value).transpose(0, 1)
    ...
    src[src == pad_value] = 0
    exit_particle[exit_particle == pad_value] = 0

    return src_mask, tgt_mask, src_padding_mask, tgt_padding_mask


def create_mask_src(src, exit_particle, pad_value, device):
    """
