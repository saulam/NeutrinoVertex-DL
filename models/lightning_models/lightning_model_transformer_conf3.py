...
Description: PyTorch Lightning model for the third configuration of the decomposing transformer.
"""
...
class LightningModelTransformerConf3(pl.LightningModule):
    def __init__(self, model, loss_fn1, loss_fn2, loss_fn3, loss_fn4, args):
        """
        Initialise the Lightning Model for the third configuration of the decomposing transformer.
    ...
    def common_step(self, batch):
        ...
        src_mask, tgt_mask, src_padding_mask, tgt_padding_mask = create_mask(hits, ekin_true_input, exit_particle, self.pad_value,
                                                                             self.device)

        # Pass the inputs and masks to the model to get predictions
        vtx_pred, ekin_pred, dir_pred, keep_iter_pred = self.model(hits, exit_particle, ekin_true_input, dir_true_input, src_mask, tgt_mask,
