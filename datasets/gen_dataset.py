...
class GenDataset(Dataset):
    ...
    def __init__(self, args, split: str = "train"):
        ...
        # LRU Cache for ZipFile objects (max 100 open files per worker)
        self._zip_cache = OrderedDict()
        ...
        for L in self.lens:
            ...
            # save the last event of each sub voxel as the test event (last valid index)
            self.test_events.append(total - 1)  # -1 because indices are 0-based
        
        self.total_events = self.__len__() # total number of different particles

    def __del__(self):
        """Close all cached ZipFiles when dataset is destroyed."""
        for zip_file in self._zip_cache.values():
            try:
                zip_file.close()
            except:
                pass
    ...
    def collate_fn(self, batch):
        ...
        if self.particle == "muon" or self.particle == "proton_exiting":
            ...
            params_batch = np.array([np.concatenate([event['pos_ini'], event['ke'], event['dir_ini']])
                                     for event in batch if event['pos_ini'] is not None])
        img_batch = torch.tensor(img_batch).unsqueeze(1).float()
    ...
    def __getitem__(self, idx):
        ...
        if self.split == "train":
            while idx in self.test_events:
                # pick a random event that's not in test set
        ...
        # Use LRU cached ZipFile or open new one
        if data_file in self._zip_cache:
            # Move to end (most recently used)
            self._zip_cache.move_to_end(data_file)
            zip_file = self._zip_cache[data_file]
        else:
            # Open new ZipFile
            zip_file = ZipFile(data_file, "r")
            self._zip_cache[data_file] = zip_file
            
            # Evict oldest if cache is full
            if len(self._zip_cache) > self._max_cache_size:
                oldest_file, oldest_zip = self._zip_cache.popitem(last=False)
                oldest_zip.close()
        ...
        if self.img_size < self.va_size:
            boundary_size = self.va_size//2 - self.img_size//2
            output['image'] = dense_image[boundary_size:-boundary_size, boundary_size:-boundary_size, boundary_size:-boundary_size]
        else:
            output['image'] = dense_image
